...
class DistributionSTParams(NamedTuple):
    ...
    def get_distributions(self, x):
        """
        Évalue les distribution des états singulet et triplet aux points x passés en argument
        et en utilisant les paramètres utilisés pour instantier le NamedTuple.
        """
    ...
    def get_visibility(self, threshold):
        """
        Évalue la visibilité d'une mesure en fonction des paramètres du tuple.
        """
    ...
    def find_optimal_threshold(self):
        """
        Tente de trouver le threshold qui permet de maximiser la visibilité en utilisant la méthode `brentq`
        de scipy.optimize. Si le threshold ne peut pas être déterminé, la fonction retourne None.
        """
...
class DistributionSTFitResult:
    ...
    - des valeurs calculées en fonction du fit
    ...
    def from_bins_hist(
        ...
        if find_threshold:
            results.threshold = results.popt.find_optimal_threshold()
        ...
        if compute_visibility and results.threshold is not None:
            ...
            fid_sin, fid_tri, vis = results.popt.get_visibility(results.threshold)
