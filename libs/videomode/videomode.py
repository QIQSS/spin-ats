...
class VideoModeWindow(QMainWindow):
    ...
    def from_ats(
        ...
        **kwargs):
        """
        ATS must have nbwindows=1
        Reshaped to (long_axis.nbpts, -1)
        """
        ...
        def acq(channel: str):
            ...
            # n_expected = int(short_axis.nbpts*long_axis.nbpts*readout_time*ats.sample_rate.get())
            # n_acq = len(res)
            # print(n_acq, n_expected)
            # if n_acq > n_expected:
            #     res = res[:, :n_expected]
            # elif n_acq < n_expected:
            #     res = np.pad(res, (0, n_expected), mode='edge')
        ...
        def saveto(data_2d):
            filename = save_path + "%T_vm_" + channel + ".hdf5"
            filename = expand_filename(filename)

            dialog = PasteDialog.PasteDialog()
            dialog.exec_()
            x_axis = np.linspace(long_axis.orig_start, long_axis.orig_stop, long_axis.nbpts)
            y_axis = np.linspace(short_axis.orig_start, short_axis.orig_stop, data_2d.T.shape[1])
            with sweep_file(
                filename,
                [long_axis.element, short_axis.element],
                [x_axis, y_axis], [channel],
    ...
    def __init__(
        ...
        fn_on_last_window_stop = lambda: None,
        ...
        self.fn_on_last_window_stop = fn_on_last_window_stop
